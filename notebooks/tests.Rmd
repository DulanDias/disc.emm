...
```{r}
matrix(log_pi, nrow = n, ncol = length(log_pi), byrow = TRUE)
```
```{r}
tabulate(z)
```
...
```{r}
cbind(log_x, matrix(rep(log_x[, 1], ncol(log_gamma) - 1), nrow = nrow(log_x)))
```

```{r}
rowSums(log_gamma + cbind(log_x, matrix(rep(log_x[, 1], ncol(log_gamma) - 1), nrow = nrow(log_x)))) - rowSums(log_gamma)
```
